...
export default class Maze extends Component<IProps, IState>{
    ...
    constructor(props: IProps) {
        super(props);
        this.props.api.getEventApi().addEventHandler('mazeLayoutChanged', this.onMazeLayoutChanged.bind(this));
        this.props.api.getEventApi().addEventHandler('playerPositionsChanged', this.onPlayerPositionsChanged.bind(this));
    }
    ...
    shouldComponentUpdate(nextProps: Readonly<{}>, nextState: Readonly<IState>): boolean {
        return false;
    }
    ...
    onInputChanged(isKeyDown: boolean, event: KeyboardEvent): void {
        ...
        if(controlKey !== undefined && this.inputMap[controlKey] !== isKeyDown) {
            ...
            this.props.api.getEventApi().sendMessageToServer('playerInputChanged', data as EventDataObject);
        }
    }
    ...
    createBufferedMaze(p5: p5Types, graphics: p5Types.Graphics): void {
        ...
        for(let x = 0; x < this.maze.length; x++) {
            for(let y = 0; y < this.maze[x].length; y++) {
                ...
                if(tile.borderTop)      graphics.line(xSrc, ySrc, xSrc + this.tileSize, ySrc);
                if(tile.borderLeft)     graphics.line(xSrc, ySrc, xSrc, ySrc + this.tileSize);
            }
        }
    ...
    draw(p5: p5Types): undefined {
        ...
        const localePlayerId = this.props.api.getPlayerApi().getLocalePlayer().getId();
        ...
        const localePlayer = players.splice(players.findIndex((p) => p.playerId === localePlayerId), 1);
        ...
        // eslint-disable-next-line no-lone-blocks
        {
            ...
            p5.background('#222');
            p5.image(this.bufferedMaze as p5Types.Graphics, -playerPos.x / this.zoom, -playerPos.y / this.zoom);
