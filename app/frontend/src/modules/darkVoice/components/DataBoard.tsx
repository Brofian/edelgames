...
export default class DataBoard extends Component<IProps, {}> {
			...
			<div className={'data-board'}>
				<div className={'scoreboard'}>
					...
					{this.props.scores
						...
						.map(this.renderPlayerScoreboardRow.bind(this))}
				</div>
			</div>
		);
	}
	...
	renderPlayerScoreboardHeader(): JSX.Element {
		return (
			<div className={'scoreboard-row scoreboard-header'}>
				...
				<div className={'scoreboard-col-name'}></div>
	...
	renderPlayerScoreboardRow(score: PlayerScore, index: number): JSX.Element {
		const player = RoomManager.getRoomMembers().find(
			(member) => member.getId() === score.playerId
		);
		...
		return (
			<div key={"player_score_" + index} className={rowClasses.join(' ')}>
				...
				<div className={'scoreboard-col-points'}>{score.score}</div>
			</div>
		);
	}
}
